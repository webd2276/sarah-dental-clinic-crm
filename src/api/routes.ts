...
import { createCalendarEvent, updateCalendarEvent, deleteCalendarEvent, checkCalendarHealth } from '../lib/google-calendar.js';
import { checkSheetsHealth } from '../lib/google-sheets.js';
 ...
 */
apiRouter.post('/webhooks/n8n-sync', async (req: Request, res: Response) => {
  try {
    ...
    // 1. Upsert Patient
    const patient = await upsertPatientByPhone(patient_name, phone_number, email);

    let booking;
    let logDesc = '';
    let logAction: 'booking_created' | 'booking_rescheduled' | 'booking_cancelled' = 'booking_created';

    if (action === 'rescheduled') {
      logAction = 'booking_rescheduled';
      if (booking_id) {
        booking = await updateBookingStatus(booking_id, {
          status: 'rescheduled',
        ...
        const existingBookings = await getBookings({ search: phone_number });
        if (existingBookings.length > 0) {
          booking = await updateBookingStatus(existingBookings[0].id, {
            status: 'rescheduled',
            appointment_date: date,
            appointment_time: time,
            notes: notes || 'Rescheduled via n8n webhook',
      ...
      if (booking_id) {
        booking = await updateBookingStatus(booking_id, { status: 'cancelled', notes });
      } else {
        const existingBookings = await getBookings({ search: phone_number });
        if (existingBookings.length > 0) {
          booking = await updateBookingStatus(existingBookings[0].id, { status: 'cancelled', notes });
        }
      }
...
apiRouter.patch('/bookings/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (existing.google_calendar_event_id) {
      if (status === 'cancelled') {
        ...
        await updateCalendarEvent(existing.google_calendar_event_id, {
          summary: `${existing.service_type} - ${updated.patient?.full_name || 'Patient'}`,
    ...
    // Log activity
    let actionType: 'booking_rescheduled' | 'booking_cancelled' = 'booking_rescheduled';
 ...
 * GET /api/patients
 */
apiRouter.get('/patients', async (_req: Request, res: Response) => {
  try {
...
apiRouter.get('/patients/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const patientDetail = await getPatientById(id);
    if (!patientDetail) {
 ...
 * GET /api/activity
 */
apiRouter.get('/activity', async (_req: Request, res: Response) => {
  try {
 ...
 */
apiRouter.get('/integrations/status', async (_req: Request, res: Response) => {
  try {
    const calStatus = await checkCalendarHealth();
    const sheetsStatus = await checkSheetsHealth();
 ...
 */
apiRouter.post('/integrations/test-webhook', async (req: Request, res: Response) => {
  try {
    const { action = 'created', patient_name = 'Test Patient', service_type = 'Teeth Cleaning', channel = 'call' } = req.body;

    const patient = await upsertPatientByPhone(patient_name, '+1 (555) 999-0000', 'test@clinic.com');
